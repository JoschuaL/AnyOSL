...
int
OSLCompilerImpl::insert_code(int opnum, const char* opname, size_t nargs,
...
OSLCompilerImpl::make_temporary(const TypeSpec& type)
{
    ...
    if (type.is_structure() || type.is_structure_array()) {
        ...
        add_struct_fields(type.structspec(), name, SymTypeTemp,
                          type.arraylength());
    }
...
void
OSLCompilerImpl::add_struct_fields(StructSpec* structspec, ustring basename,
                                   SymType symtype, int arraylen, ASTNode* node,
                                   ASTNode* init)
{
    if (init) {
...
int
ASTNode::emitcode(const char* opname, Symbol* arg0, Symbol* arg1, Symbol* arg2,
                  Symbol* arg3)
{
...
{
    OSL_ASSERT_MSG(0, "Unimplemented codegen for %s (%s)", nodetypename(),
                   opname() ? opname() : "");
    return nullptr;
}
        ...
        if (expr()) {
            ...
            if (dest != retloc) {
                if (retloc->typespec().is_structure()) {
                    ...
                    codegen_assign_struct(structspec,
                                          ustring(retloc->mangled()),
                                          ustring(dest->mangled()), NULL, true,
...
ASTcompound_initializer::codegen(Symbol* sym)
{
    ...
    if (m_typespec.is_structure_based()) {
        ...
        return codegen_struct_initializers(initlist(), sym,
                                           true /*is_constructor*/);
    }
...
void
ASTNode::codegen_assign_struct(StructSpec* structspec, ustring dstsym,
                               ...
                               bool paraminit)
{
    for (int i = 0; i < (int)structspec->numfields(); ++i) {
        ...
        m_compiler->struct_field_pair(structspec, i, dstsym, srcsym, dfield,
                                      ofield);
        if (paraminit) {
...
bool
ASTNode::one_default_literal(const Symbol* sym, ASTNode* init, std::string& out,
                             string_view sep) const
{
            ...
            out += Strutil::sprintf("%.9g%s%.9g%s%.9g", f, sep, f, sep, f);
                   ...
                   && (init->nodetype() == ASTNode::type_constructor_node
                       || (init->nodetype() == ASTNode::compound_initializer_node
                           && static_cast<ASTcompound_initializer*>(init)
                                  ->canconstruct()))) {
                                                ...
                                                string_view separator) const
{
    ...
    if (sym->typespec().is_structure()
        || sym->typespec().is_structure_array()) {
...
ASTNode::codegen_initlist(ref init, TypeSpec type, Symbol* sym)
{
    ...
    if (type.is_array() && !type.is_closure_based()
        ...
        for (int i = 0; init && i < type.arraylength(); ++i) {
            ...
            case compound_initializer_node:
                ctor = static_cast<ASTcompound_initializer*>(expr)
                           ->canconstruct();
                break;
            default: break;
            }
    ...
    for (int i = 0; init; init = init->next(), ++i) {
        if (sym->typespec().is_structure()
            ...
            // Nested structure
            ASTNode::ref finit
                = ((ASTcompound_initializer*)init.get())->initlist();
...
Symbol*
ASTNode::codegen_struct_initializers(ref init, Symbol* sym, bool is_constructor,
                                     Symbol* arrayindex)
{
    // If we're doing this initialization for shader params for their
    // init ops, we need to take care to set the codegen method names
    ...
    if (init->nodetype() != compound_initializer_node && !is_constructor) {
        ...
        if (initsym != sym) {
            ...
            codegen_assign_struct(structspec, ustring(sym->mangled()),
                                  ustring(initsym->mangled()), arrayindex, true,
                                  0, paraminit);
        }
        return sym;
    }
    ...
    for (int i = 0; init && i < structspec->numfields();
        ...
        if (fieldsym->typespec().is_structure_based()
            && (init->nodetype() == type_constructor_node
                || init->nodetype() == compound_initializer_node)) {
            bool ctor = init->nodetype() == type_constructor_node
                            ? true
                            : static_cast<ASTcompound_initializer*>(init.get())
                                  ->canconstruct();
        ...
        if (init->nodetype() == compound_initializer_node
            ...
            codegen_initlist(((ASTcompound_initializer*)init.get())->initlist(),
                   ...
                   && static_cast<ASTfunction_call*>(init.get())
                          ->is_struct_ctr()) {
            codegen_struct_initializers(
                static_cast<ASTfunction_call*>(init.get())->args(), fieldsym,
                true, arrayindex);
        } else {
...
Symbol*
ASTindex::codegen(Symbol* dest, Symbol*& ind, Symbol*& ind2, Symbol*& ind3)
{
    ...
    if (lv->typespec().is_array()) {
        if (index3()) {
            // matrixarray[a][c][r]
            Symbol* tmp = m_compiler->make_temporary(
                lv->typespec().elementtype());
...
void
ASTindex::codegen_assign(Symbol* src, Symbol* ind, Symbol* ind2, Symbol* ind3)
{
    Symbol* lv = lvalue()->codegen();
    if (!ind)
        ind = index()->codegen();
...
ASTbinary_expression::codegen(Symbol* dest)
{
    if (m_function_overload) {
        ...
        ASTfunction_call fc(m_compiler, funcname, args.get(),
                            m_function_overload);
        fc.typecheck(typespec());
...
std::string op_to_string(int op_int){
...
std::string artic_type_string_to_string(TypeSpec typeSpec){
    auto tstring = typeSpec.artic_string();
    size_t pos;
...
ASTfunction_call::codegen(Symbol* dest)
{
    ...
    ASTNode* form = is_user_function() ? user_function()->formals().get()
                                       : NULL;
    ...
    if (is_user_function()) {
            ...
            if (ftype.is_structure() || ftype.is_structure_array()) {
                ...
                if (a->nodetype() == variable_ref_node
                    ...
                    struct_pair_all_fields(
                        ftype.structspec(), ustring(f->sym()->mangled()),
        ...
        // Emit the actual op
        emitcode(isclosure ? "closure" : m_name.c_str(), argdest.size(),
                 (argdest.size()) ? &argdest[0] : NULL);
...
void
ASTfunction_call::struct_pair_all_fields(StructSpec* structspec, ustring formal,
                                         ustring actual, Symbol* arrayindex)
{
    for (int fi = 0; fi < (int)structspec->numfields(); ++fi) {
        const StructSpec::FieldSpec& field(structspec->field(fi));
        const TypeSpec& type(field.type);
